...
const MONTH_MS = 6000;                   // 1ヶ月 = 5フェーズ × 1.2s
// エンジンに実在するフェーズだけ。労働と生産は未実装なので並べない。
const PHASES = ['人口', '所得', '消費', '財政', '記録'];
const N_AGENTS = 100;
const MAX_CARS = 22;
...
const C = {
  ...
  poor: 0xC0392B,          // 困窮世帯(基礎バスケットが買えない)
...
const SKY = [
  ...
  [0xFF8E6E, 0xFFCBA6], [0x232750, 0x4E4E86],
];
// 5フェーズぶん。朝(人口)→ 昼(所得)→ 夕(消費)→ 宵(財政)→ 夜(記録)
const SUN_COL = [0xFFF3D0, 0xFFFFFF, 0xFFD9A0, 0xFF9E6E, 0x8C90D8];
...
let flows = null, accounts = null, lifeEvents = null;
let stalls = 0;   // 立ち往生を解消した回数(観測用)
...
function paintSky(phase, frac) {
  const a = SKY[phase], b = SKY[(phase + 1) % SKY.length];
...
function seriesAt(m) {
  ...
  return { unemployment: s.失業率[i], cpi: s.物価指数[i], gdp: s.GDP[i], gdp0: s.GDP[0],
           debt: s.政府債務[i], balance: s.財政収支[i], gini: s.所得ジニ[i] };
}
...
function applyMonth(m) {
  ...
  // --- 街の1体 = 標本世帯の1つ。表示は集計値からの逆算ではなく、実データ -------
  const 街 = data.街;
  const 月 = 街 && 街.月ごと[Math.min(m, 街.月ごと.length - 1)];

  if (月) {
    const col = new THREE.Color();
    agents.forEach((a, i) => {
      const 世帯 = 月.世帯[i % 月.世帯.length];      // [所得, 貯蓄, 困窮, 存続]
      a.所得 = 世帯[0]; a.貯蓄 = 世帯[1];
      a.困窮 = 世帯[2]; a.存続 = 世帯[3];
      // 就業は「所得があるか」。集計値からの割り当てではない
      a.employed = 世帯[0] > 0;
      a.vis = 世帯[3] ? 1 : 0;                       // 世帯が消えたら街から消える
      inst.body.setColorAt(i, col.setHex(
        !世帯[3] ? C.jobless : 世帯[2] ? C.poor : a.employed ? CLOTHES[a.cloth] : C.jobless));
    });
    inst.body.instanceColor.needsUpdate = true;
    joblessCount = agents.filter(a => !a.employed && a.存続).length;

    // --- 資金の流れ。矢印の太さ = 実際の金額 ---------------------------------
    flows = 月.資金の流れ;
    accounts = 月.口座;
    lifeEvents = { 死亡: 月.死亡, 出生: 月.出生, 相続: 月.相続 };
  }

  setSign('debt', '債務 ' + (d.debt / 1e12).toFixed(2) + '兆');
  setSign('jobs', '無収入 ' + joblessCount + '/' + agents.length + '世帯',
    { bg: joblessCount > agents.length * 0.12 ? '#FE806F' : '#FFC93D' });
  refreshFx();
}
...
function onPhase(p) {
  ...
  // ① 所得フェーズ: 台帳で FIRMS→HOUSEHOLDS が動く月に、所得のある世帯だけが金を持つ
  if (p === 1) for (const a of agents) if (a.employed && !a.carry) a.carry = 1;

  for (const a of agents) {
    if (a.存続 === false) continue;              // 消えた世帯は動かない
    if (!FREE(a)) continue;                      // 移動中の人の行き先は奪わない

    if (p === 0) go(a, a.home);                  // ⓪人口 — 加齢・出生・死亡・独立・相続
    else if (p === 1) go(a, a.employed ? a.work : hello);   // ③所得
    else if (p === 2) {                          // ④消費
      // 困窮世帯は基礎バスケットしか買えない。店に行かず家にいる
      go(a, a.困窮 ? a.home : a.employed || a.貯蓄 > 0 ? a.store : a.home);
    }
    else if (p === 3) {                          // ⑤政府 — 税・年金・給付
...
function separateAgents() {
  ...
  //
  // ただし **目的地から遠ざかる向きには戻さない**。
  // 次の点が道路の向こう側にある歩行者を毎フレーム引き戻すと、
  // 「0.2進む → 引き戻される」を繰り返して、その場に釘付けになる。
  // 実際これで歩行者の4分の1が横断歩道の手前で止まっていた。
  for (const a of agents) {
    if (a.vis <= 0.05 || a.crossing) continue;
    const wp = a.path && a.path[a.pi];
    const tx = wp ? (wp.x ?? wp[0]) : null;
    const tz = wp ? (wp.z ?? wp[1]) : null;

    for (const z of AVE_Z) {
      const dz = a.z - z;
      if (Math.abs(dz) >= ROAD_H + 0.3) continue;
      const 戻す先 = z + Math.sign(dz || 1) * (ROAD_H + 0.3);
      // 目的地が反対側にあるなら、押し戻すと遠ざかる。触らない
      if (tz !== null && Math.abs(tz - 戻す先) > Math.abs(tz - a.z)) break;
      a.z = 戻す先; break;
    }
    for (const x of ST_X) {
      const dx = a.x - x;
      if (Math.abs(dx) >= ROAD_H + 0.3) continue;
      const 戻す先 = x + Math.sign(dx || 1) * (ROAD_H + 0.3);
      if (tx !== null && Math.abs(tx - 戻す先) > Math.abs(tx - a.x)) break;
      a.x = 戻す先; break;
    }
  }
}
...
function updateAgents(dts) {
  // 立ち往生の判定用に、フレーム開始時の座標を控える。
  // 判定は separateAgents() の後で行う — 歩いた分を押し分けが打ち消す形で
  // 固まるので、advance() の直後に見ても「動いた」ように見えてしまう。
  for (const a of agents) { a._px = a.x; a._pz = a.z; }
  ...
  // --- 立ち往生の解消 --------------------------------------------------------
  // 歩行と押し分け(または歩道への引き戻し)が綱引きになると、その場で固まる。
  // 赤信号でもないのに1.5秒進めなければ、その点を諦めて次へ送る。
  // 原因を全部潰すより、詰まったら自力で抜け出せるほうが街として正しい。
  for (const a of agents) {
    if (a.state !== 'walk' || a.waitSignal || a.vis <= 0.05) { a.stallT = 0; continue; }
    if (Math.hypot(a.x - a._px, a.z - a._pz) > 1e-3) { a.stallT = 0; continue; }
    a.stallT = (a.stallT || 0) + dts;
    if (a.stallT <= 1.5) continue;
    a.stallT = 0;
    a.crossing = false;
    if (a.path && a.pi < a.path.length - 1) a.pi++;      // その点を諦めて次へ
    else { a.state = 'idle'; a.rest = 0; }               // 終点なら用事をやり直す
    stalls++;
  }
...
function setRun(result, which) {
  ...
  data = result.実行結果[variant] || result.実行結果.法律なし;
  month = 0; phaseT = 0; applyMonth(0); onPhase(0);
}
function setVariant(w) { if (run && run.実行結果[w]) { variant = w; data = run.実行結果[w]; applyMonth(month); } }
function seek(m) {
  month = Math.max(0, Math.min((data ? data.月数.length : 1) - 1, m | 0));
  phaseT = 0; applyMonth(month); onPhase(phaseIndex());
  if (onMonth) onMonth(month, phaseIndex());
}
const phaseIndex = () => Math.min(4, Math.floor(phaseT / (MONTH_MS / 5)));
function start() { if (!raf) { last = performance.now(); raf = requestAnimationFrame(tick); } }
function stop() { if (raf) { cancelAnimationFrame(raf); raf = null; } }
function setPlaying(p) { playing = p; }
/** 再生速度。1ヶ月の長さを 1/倍率 にする。信号と歩行も同じだけ速くなる */
let speed = 1;
function setSpeed(x) { speed = Math.max(0.25, Math.min(20, +x || 1)); }

function tick(now) {
  const raw = Math.min(64, now - last); last = now;
  const dt = raw * speed;                    // 速度は時間の進み方そのものを変える
  const dts = dt / 1000;

  if (playing && data) {
    const prev = phaseIndex();
    phaseT += dt;
    while (phaseT >= MONTH_MS) {             // 10倍速だと1フレームで複数月進む
      phaseT -= MONTH_MS;
      month = (month + 1) % data.月数.length;
      applyMonth(month);
    }
    const p = phaseIndex();
    if (p !== prev) onPhase(p);
  ...
  const p = phaseIndex(), frac = (phaseT % (MONTH_MS / 5)) / (MONTH_MS / 5);
  paintSky(p, frac);
  const dayT = (p + frac) / 5, ang = Math.PI * (0.08 + dayT * 0.9);
  ...
  const night = p >= 3 ? (p === 3 ? 0.4 + frac * 0.5 : 0.9) : (p === 2 ? frac * 0.4 : 0);
...
export const Town = {
  ...
  _jump: (m, ph) => { seek(m); phaseT = ph * (MONTH_MS / 5) + 40; applyMonth(m); onPhase(ph); },
  /** 交差点を真上から見る(横断歩道の検分用) */
  _cross: (jx = 2, iz = 1) => {
    autoOrbit = false;
  ...
  /** 歩行者の生の状態。詰まりを機械的に検出するための観測窓 */
  _agents: () => agents.map((a, i) => ({
    i, state: a.state, x: +a.x.toFixed(2), z: +a.z.toFixed(2),
    pi: a.pi, n: a.path ? a.path.length : 0,
    wait: +(a.wait || 0).toFixed(2), waitSignal: !!a.waitSignal,
    crossing: !!a.crossing, vis: a.vis, employed: a.employed,
    dest: a.dest ? (a.dest.name || a.dest.kind) : null,
  })),
  /** 秒だけ駒送りして、動かなかった歩行者を数える */
  _stuck: (sec = 20) => {
    const before = agents.map(a => a.x + ',' + a.z);
    Town._step(sec);
    const out = { 動かなかった: [], 信号待ち: 0, 経路なし: 0 };
    agents.forEach((a, i) => {
      if (a.waitSignal) out.信号待ち++;
      if (!a.path) out.経路なし++;
      if (before[i] === a.x + ',' + a.z && a.vis > 0.05 && a.state === 'walk')
        out.動かなかった.push({ i, state: a.state, pi: a.pi, n: a.path ? a.path.length : 0,
                              waitSignal: !!a.waitSignal, x: +a.x.toFixed(1), z: +a.z.toFixed(1) });
    });
    out.合計 = agents.length;
    return out;
  },
  /** 1フレームずつ座標を記録する */
  _trace: (i, n = 6) => {
    const a = agents[i], out = [];
    for (let k = 0; k < n; k++) {
      out.push({ x: +a.x.toFixed(3), z: +a.z.toFixed(3), pi: a.pi, st: a.state,
                 w: +(a.wait || 0).toFixed(3), sig: !!a.waitSignal });
      updateSignals(simClock += 1 / 60);
      updateAgents(1 / 60);
    }
    out.push({ x: +a.x.toFixed(3), z: +a.z.toFixed(3), pi: a.pi, st: a.state });
    return out;
  ...
  _probe: (i) => {
    const a = agents[i];
    const wp = a.path && a.path[a.pi];
    const 前 = { x: +a.x.toFixed(2), z: +a.z.toFixed(2), pi: a.pi };
    for (let k = 0; k < 60; k++) { updateSignals(simClock += 1/60); updateAgents(1/60); }
    return {
      state: a.state, wait: +(a.wait||0).toFixed(3), vis: a.vis,
      speed: a.speed, rest: a.rest, moving: a.moving,
      進む量: a.speed * (1/60),
      経路の長さ: a.path ? a.path.length : 0,
      次の点: wp ? { x: +(wp.x ?? wp[0]).toFixed(2), z: +(wp.z ?? wp[1]).toFixed(2), cross: wp.cross || null } : null,
      距離: wp ? +Math.hypot((wp.x ?? wp[0]) - 前.x, (wp.z ?? wp[1]) - 前.z).toFixed(2) : null,
      前, 後: { x: +a.x.toFixed(2), z: +a.z.toFixed(2), pi: a.pi },
      crossing: !!a.crossing, waitSignal: !!a.waitSignal,
      toPlatform: !!a.toPlatform, boardSt: !!a.boardSt, trainTo: !!a.trainTo,
    };
  },
  _stalls: () => stalls,
  _sig: () => {
  ...
  init, setRun, setVariant, seek, setPlaying, setSpeed, start, stop, setLaw,
  get month() { return month; },
  get phase() { return phaseIndex(); },
  get phaseName() { return PHASES[phaseIndex()]; },
  get length() { return data ? data.月数.length : 0; },
